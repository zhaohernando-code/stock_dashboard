...
- 纸面追踪页面使用同一策略表格、账户曲线和交易明细展示三个 V3 角色与一个外部信息对照，共四个角色，全部从 2026-07-08、20 万本金起算。
- 外部信息对照在 2026-07-08 至 2026-08-07 的冻结历史标记为 `retrospective_pit_backfill`；2026-08-08 至本次恢复发布前缺失的账户段按固化日源重放并标记为 `post_activation_pit_reconstruction`，不得冒充真实前向；发布后的新交易日才标记为 `true_forward_shadow`。各段在同一曲线连续展示，但证据性质不得混淆。
- 外部信息仍不能独立触发买入，只能延长已有持仓退出；它不替换 V3，也不改变其他三个账户的历史账本。
- 本决定取代 2026-08-14 将 Round75 完全排除在 V3 纸面比较之外的界面解释。

[2026-08-14T16:10:00+08:00] Retired Rank5 shadow progress is removed from the V3 paper page:

- 纸面追踪页不再展示“Rank5 已停用 · 影子观察”卡片；可比较账户统一通过既有策略表格、账户曲线和交易明细展示，不为影子研究另开模块。
...
[2026-08-06T19:13:00+08:00] External-context Raw/Silver/PIT mechanics are ready, but live provider data remains gated:

- Added content-addressed immutable Raw records, versioned Silver records, append-only PIT knowledge versions, deterministic manifests and a strictly local replay CLI. Every artifact path, SHA-256 and byte size is verified before a replay can return data.
- PIT replay selects `available_from <= decision_cutoff < available_to`; a later revision closes the previous version. Timezone-free timestamps, backdated `first_seen_at`, multiple visible revisions, path traversal, hash mismatch and immutable path collisions are hard failures.
- A synthetic SEC-style two-revision fixture proves mechanics only: six files were materialized under manifest `external-context-manifest-3b79b96cadbda14d0eb4fec6`; separate cutoffs selected revisions `1` and `2`, both with offline hash verification and no network use. It is not live SEC evidence and does not change provider readiness.
- Credential-like Raw fields are rejected. Tushare `major_news` and Massive/Benzinga Raw materialization are code-blocked until article storage and frozen-replay rights pass. V3 remains unchanged and full713 external-context backtesting remains forbidden.
- A separate live aggregate-only repeat probe made two immediate pulls without retaining provider rows. `stock_st` returned `254/254`; SPX, IXIC and HKTECH returned `770/770`, `770/770` and `752/752`; every response-row digest matched. This clears only same-session transport stability, not multi-day corrections, storage rights, independent fallback or offline Raw replay.

[2026-08-06T20:30:00+08:00] External-context channel PoC is realigned to the user-frozen candidate set and strict time lineage:

- Provider registry V2 removes preassigned primary/fallback roles. Official facts, global market and professional news each require two ready providers from independent groups before the layer can advance; documentary score never overrides a hard gate.
- The V2 set is exactly the planned parallel set: CNINFO, SSE, SZSE, SEC EDGAR and company IR for official facts; Wind plus the named low-cost challengers for global markets; LSEG and RavenPack plus the named low-cost challengers for professional news. Massive, Benzinga, GDELT and FRED/ALFRED remain only in historical V1 records and cannot be purchased or promoted through V2.
- All four lineage fields are mandatory: `published_at`, locally captured `first_seen_at`, true client `available_at`, and `revision_id`. Current audit has 13 candidates: 0 ready, 8 pending and 5 eliminated. Tushare `stock_st`, `index_global`, `major_news`, Tiingo EOD and Tiingo News are eliminated because `revision_id` is not explicit; transport availability or a high documentary score cannot restore them.
- The frozen PoC event set includes 96 A-share official-event slots, 40 overseas issuer fact slots, eight global instruments over full713+warmup, four stress windows, 136 official anchors and a 200-item manual news review. No entitled vendor sample is currently available, so the event IDs are preregistered but not yet materialized.
- The external residual policy is now executable research code but is not wired into V3. It preserves lambda zero, uses `z(core) - clip(lambda * z(adverse residual), -cap, +cap)`, prevents positive news from creating eligibility, lets major negative official events gate risk, restricts global/sector/individual effects to their own channels, and selects the smallest all-gates-pass weight within one standard error of the best out-of-sample result.
- Full713 external-context weight backtesting remains forbidden until entitled Raw/Normalized/PIT samples pass rights, time, revision, mapping, repeat-hash and network-free replay gates. The next external action is to send the prepared vendor trial request after the user supplies/authorizes the required business contacts; no outreach or purchase was made in this change.

[2026-08-06T21:10:00+08:00] Public news becomes the first implementation path under a sub-2-GiB personal-computer contract:

- Public sources may start before commercial trials: CNINFO/SSE/SZSE and government notices are the A-share official-fact layer; SEC EDGAR and company IR cover overseas original facts; GDELT event/GKG metadata and official RSS are discovery inputs. Commercial feeds remain later challengers rather than prerequisites.
- News ingestion is summary-only. Immutable Raw rejects article bodies, HTML, full text and arbitrary provider payloads; it accepts only bounded headline/summary/source/language/content-hash and summary-lineage fields. A summary is capped at 1,000 characters and 4,096 UTF-8 bytes, and model-generated summaries must identify the model version.
- Relevance is computed locally before storage, with channel-specific interpretable weights and hard component floors. Records below the 0.65 total threshold, missing required individual/sector mappings, duplicate by source URL + content hash + headline, or above daily global/sector/individual quotas are excluded before Raw/Normalized/PIT materialization.
- Compact JSON is used for news. The entire news artifact root, including Raw, Normalized/Silver, PIT and manifests, targets 1.5 GiB and checks every immutable write against a 1.75 GiB hard cap, leaving explicit headroom below the user's 2 GiB ceiling. The cap fails closed and cannot be silently raised or bypassed.
- This is a storage/relevance contract and tested materialization path, not a populated full713 dataset. V3 and paper trading remain unchanged; weight backtesting stays blocked until public-source adapters materialize PIT-safe history.

[2026-08-06T21:20:00+08:00] Public-source adapters retain only material summaries and must clear source-rights gates before full713 acquisition:

- CNINFO is fetched through its official HTTPS query endpoint without announcement-body downloads. The live 2024 sample across `600519`, `000001`, `600589`, and `002415` fetched `467/467` reported rows, excluded `299` routine/low-increment titles, retained `168` material announcements, and collapsed same-symbol/same-day/same-category duplicates into `112` immutable event packages. The `600519` sample exposed and fixed an upstream pagination inconsistency (`68` total announcements versus an initial two-page/60-row indication); page count is now `max(totalpages, ceil(total/30))`.
- CNINFO `announcementTime` values observed in the live sample encode calendar-day midnight rather than a trustworthy intraday dissemination time. Historical availability is therefore frozen at `23:59:59.999999 Asia/Shanghai`, so same-day decisions cannot see the announcement. Routine board/supervisory resolutions, meeting materials, legal opinions, English duplicate reports, director/audit appendices, and results-briefing notices are filtered before storage. At most six titles/IDs are retained in a same-day materiality group; no PDF, HTML or body text is stored.
- GDELT daily Event archives are read from the official Google Cloud Storage HTTPS bucket into bounded memory and are never persisted. The final live `2026-08-04` archive was `6,569,696` compressed bytes and `102,313` rows; URL relevance plus deduplication produced `121` candidates, and fixed topic maxima retained `10` discovery summaries (`4` semiconductor, `2` telecommunications, `4` trade restriction, `0` US macro). Generic utility-price “rate hike” false positives, numeric CMS slugs, tariff-only crowding, an incorrect country-column index, and an initially optimistic next-day availability assumption were found in the probes and corrected before acceptance.
- GDELT remains global-state discovery metadata, not a professional-news substitute and not a source of independent buy triggers. Daily `DATEADDED` has no publication time, so historical records are conservatively delayed until the second following UTC day, after a full uncertain publication day. CNINFO and GDELT real samples both materialized through immutable Raw/Silver/PIT and replayed with hash verification passed and `network_used=false`.
- A deterministic historical-price eligibility canary then sampled `24` unique symbols over `2023-06-13`, `2024-06-05`, `2025-06-05`, and `2026-05-26` without reading current name/status/board fields. All `24/24` 31-day requests were complete: `242` source announcements became `63` material titles and `42` same-day event packages after a second manual-review-driven attachment filter. The `42` titles had no obvious remaining topic mismatch; immutable replay used `224,015` bytes, selected `42/42`, passed hashes and used no network. Historical ST/retirement status remains explicitly unverified rather than backfilled from today.
- The four-symbol planning estimate is approximately `1.3183 GiB` for three calendar years of CNINFO event packages plus daily GDELT discovery summaries, under the `1.5 GiB` target and `1.75 GiB` hard cap. This is not authorization for full acquisition. GDELT explicitly permits unlimited/unrestricted use with attribution, but CNINFO public browsing does not establish bulk historical storage/replay rights; full acquisition is blocked pending formal CNINFO Data Service terms or written confirmation. V3 scoring, paper tracking, runtime policy and external weights remain unchanged.

[2026-08-06T21:45:00+08:00] Personal public-source acquisition is checkpoint-ready; GDELT is demoted after a seven-day relevance review:

- The user explicitly authorized personal internal use and storage where no prohibition is stated, with attribution `hernando_zhao`. The acquisition contract therefore permits compact title/identifier/metadata retention for personal research, forbids redistribution and article-body retention, and records that this is an operational assumption rather than legal advice or a source-license conclusion.
- A deterministic CNINFO plan covers `3,009` historically price-eligible main-board symbols as `9,027` at-most-366-day tasks. It uses only historical unadjusted closes for the `<=200 CNY` screen and does not backfill current name, board or risk status. The plan is frozen but the full dataset is not yet materialized.
- A real two-symbol/six-window dry run completed in three bounded batches. It retained `125` official-event packages in `660,065` bytes. All six immutable inputs and manifests replayed offline with `125/125` records selected and hashes passed; rerunning the completed plan made zero network requests. This clears checkpoint, interruption recovery, idempotence and root-cap mechanics.
- The same seven GDELT archive days (`2026-05-20` through `2026-05-26`) scanned `693,371` rows and `42,936,499` compressed bytes in memory without persisting archives or article bodies. Headline-only topic matching, topic-plus-normalized-headline deduplication and explicit quality exclusions reduced the selected daily sum from `77` to `67`, with `65` cross-day records and `76` low-quality matches rejected.
- Manual review still found sector-adjacent property/financing, consumer 5G, investment-commentary and stale/recycled stories. GDELT therefore fails the direct strategy-feature and professional-news gates; it remains a bounded discovery backup only. V3, paper tracking and runtime are unchanged, and external-weight backtesting remains blocked until full-window official facts and a qualified professional-news primary are available.
- Before the persistent run, rate limiting was tightened from one delay per symbol-window task to one delay before every real HTTP GET/POST, including pagination; the stock map is fetched once and cached for each invocation. The persistent canary now covers `11/9,027` tasks with `43` rate-gated requests across two invocations, zero failures and `196` retained events. Offline replay verified `588` files, selected `196/196`, passed all hashes and used no network. The durable data root is under runtime data artifacts and currently uses `3,826,359` bytes. A shared incremental storage counter now scans disk once per invocation and updates on each write, avoiding task-by-task full-root rescans as the dataset grows.
- A compact official-policy shortlist is frozen for the next PoC: Federal Reserve RSS/archive for global macro; SEC EDGAR for overseas issuer facts; Federal Register and BIS for export-control rules; USTR for trade policy; and MOFCOM official announcements for Chinese trade/export-control policy. These are high-authority fact inputs, not a relabeling of public metadata as professional news. Each must still pass full713 history, timestamp, revision, repeat-hash, offline replay and at least 95% manual topic-precision gates.
- Research will be separable by construction: official facts plus global-market state first, then official-policy incremental value, then a professional-news challenger when an entitled high-quality feed exists. A missing paid-news feed therefore does not contaminate the official-only ablation, but no result may be described as a combined news strategy until that challenger layer passes.

[2026-08-06T23:35:00+08:00] Two compact U.S. official-policy sources are full713 replay-ready, while CNINFO relevance moves to v2:

- Federal Reserve monetary-policy archives now retain four immutable raw page checkpoints and 55 compact events from `2023-06-13` through `2026-05-26`. Live HTML transport hashes are not a valid repeat gate because Cloudflare injects a new challenge token per request; normalized selected-event hashes and all 55 revision hashes repeat, while the checkpoint rerun uses zero network requests.
- Federal Register policy metadata now retains 46 immutable API page checkpoints and 175 deduplicated strategic-policy events. A full title/agency/abstract review removed patent disputes, administrative notices, unrelated product controls and false semiconductor mappings. The retained set has 129 trade/export-control, 48 semiconductor-policy and 13 telecommunications-policy tags; relevance does not imply high impact, which remains a later weight input.
- Both sources are materialized as Raw/Silver/PIT manifests. Offline replay at a historical cutoff and after the full713 boundary verified 690 files, passed every hash and used no network. The shared personal external-context root is about 27.0 MB under the 1.75 GiB hard cap. This clears an official-policy research ablation only; it does not qualify professional news, global-market data or a V3 weight.
- CNINFO progressed from 11 to 36 of 9,027 tasks with zero failures. The second batch added 593 event packages and verified 1,779 files offline. Review of all 789 retained v1 packages found 84 routine governance artifacts, so `cninfo_title_materiality.v2` now excludes management policies, routine fund-occupancy tables, routine independent-director opinions and annual related-party transaction estimates before storage.
- V3, paper tracking, live candidates and runtime policy remain unchanged. Full713 external-weight backtesting remains blocked until CNINFO coverage and a PIT-safe global-market layer are materialized; independent relevance annotation remains a formal gate beyond Codex's full-population review.

[2026-08-06T23:55:00+08:00] CNINFO title relevance v6 passes the frozen 200-item research sample and becomes the acquisition rule:

- Persistent acquisition now covers `111/9,027` tasks, representing `37` deterministically selected symbols with all three full713 windows complete. One `ChunkedEncodingError` on `603717.SH` left no partial checkpoint and recovered on the next attempt. Each task now permits at most two transient network attempts; rate limiting still applies to every real request.
- A reusable curation-audit command re-evaluates every frozen older input under the active rule without rewriting or duplicating Raw artifacts. Across `2,354` stored packages, v6 retains `2,004` and lists `350` exact event-version exclusions. The exclusion overlay is deterministic and hashed.
- The category-stratified, seeded 200-title review was iterative and evidence-driven. v3 exposed about 14% procedural noise; v4 removed routine debt/governance/occupancy/adviser documents; v5 reached about 98.5% obvious-topic precision; v6 removed the final company-bond rename, adviser-commitment and sponsor-summary classes. The deterministic replacement rows had no obvious mismatch, so v6 is frozen to avoid review-set overfitting.
- This passes a Codex operator-style research relevance gate, not independent human annotation and not predictive-value validation. Repeated pledges, financial reports and similar events remain because they are relevant occurrences; their importance, novelty and overlap with core stock data must be handled by residualization and bounded dynamic weighting.
- Linear storage projection is about `1.070 GB` for the full plan, below the `1.5 GiB` target and `1.75 GiB` hard cap. The projection is preflight evidence only; every immutable write continues to fail closed against the actual root cap.

[2026-08-07T00:05:00+08:00] Global-market vendor files now have a fail-closed PIT import contract, but no vendor has qualified:

- The global basket is frozen as SPX, IXIC, a semiconductor index, HKTECH, HSI, USD/CNH, US10Y and WTI over full713 plus warmup. A local validator now requires an entitled export, explicit local replay approval, provider-supplied revision IDs, timezone-aware lineage, ordered observation/publication/availability/first-seen/retrieval timestamps, valid OHLC and a frozen instrument identifier.
- Locally generated content hashes and stable instrument-date keys cannot masquerade as provider revisions. Tushare `index_global` and Tiingo EOD therefore remain eliminated until provider revision lineage exists; Wind remains pending an entitled export, field dictionary, revision-semantics evidence and local replay rights.
- A validated import can materialize through the existing immutable Raw/Silver/PIT and offline replay path, but this change is contract readiness only. It does not establish full713 coverage, qualify any provider, authorize a purchase or change V3.

[2026-08-07T00:05:00+08:00] CNINFO relevance v8 is frozen after an independent acquisition batch, not after further reuse of the old population:

- A new `25/25` task batch completed with `88` rate-gated requests, zero failures and zero transient retries. Persistent progress is `136/9,027` tasks: `45` symbols have all three windows, one symbol has one partial next window, and `8,891` tasks remain.
- Re-auditing all `2,752` stored packages under v8 retains `2,384` and emits `368` exact event-version exclusions with hash `00ba8ea8c398f05082b8850a7e6b08abed3417bfd4c535a9843b57732141c05d`. The new batch exposed and then removed narrowly defined procedural classes: annual related-party estimates, rating reports, allocation results, adviser commitments, exchange-review meeting arrangements and transaction-process representations.
- The expanded deterministic category-stratified 200-title sample has no obvious topic mismatch under a Codex operator-style review. This remains neither independent human annotation nor predictive-value evidence. Future relevance-rule changes require new acquisition data rather than another pass over this review sample.
- The persistent research root uses about `38.6 MB`; the updated linear full-plan projection is about `1.023 GB`, below the `1.5 GiB` target and `1.75 GiB` hard cap. V3, paper tracking and live candidates remain unchanged, and weight backtesting is still blocked pending a qualified global-market export and broader PIT coverage.

[2026-08-07T00:10:00+08:00] External-context ablation readiness is now an executable offline gate:

- The readiness command replayed all `136` manifests at the frozen post-full713 cutoff, verified `8,946` immutable files, applied the v8 exact CNINFO exclusion overlay and used no network. The resulting bundle is `external-context-ablation-bundle-397ffc88bd0c65dfd73de86f` with replay digest `397ffc88bd0c65dfd73de86fae1aec17ba6d321bd2977f4225e46cbab406d9a4`.
- The two official U.S. policy providers contribute `230` records and are ready only for data-quality/sample research. CNINFO contributes `2,752` stored records and `2,384` after curation, but task coverage remains `136/9,027`. No qualified global-market manifest exists.
- The gate therefore keeps the permanent stock-only `lambda=0` baseline, labels the official-policy sample as research-only, and blocks both full713 official-fact and official-plus-global weight backtests. Professional news remains a later incremental challenger and does not block the official-plus-global ablation design.

[2026-08-07T00:15:00+08:00] The durable CNINFO checkpoint advances to 50 complete symbols under relevance v9:

- Fourteen additional tasks completed with `55` rate-gated requests, zero failures and zero retries, yielding exactly `150/9,027` tasks and `50` symbols with all three windows; no symbol is left partially materialized at this checkpoint.
- Full review of the 197 newly acquired v8 records exposed written report confirmations, continuing-supervision annual reports, regulatory-work-letter replies, performance briefings, procedural related-party rules, historical compliance summaries and small-financing authorizations. v9 removes these narrow classes. Across all `2,949` stored records, the v9 overlay retains `2,563`, excludes `386`, and hashes the exclusions as `012f8957ad67388d651f344cda9b97b0b6d7d83a3b3376fba26823cdaf754c52`.
- The regenerated category-stratified 200-title sample has no obvious topic mismatch under Codex review. Independent human annotation and predictive value remain unproven. Further title-rule changes require another new acquisition population.
- The final offline readiness bundle is `external-context-ablation-bundle-7c9c55ffe2cf0c5c9e1ba010`: `148` non-empty manifests, `9,537` verified files, no network, and replay digest `7c9c55ffe2cf0c5c9e1ba010b2868e949ead0ecd48fd2f8aa2adb1e970e458d1`. Current root size is about `40.0 MB`; projected full-plan size is about `0.994 GB`.

[2026-08-07T01:05:00+08:00] Full CNINFO acquisition is assigned to a resumable ashare-dashboard background component:

- The need belongs to the existing ashare-dashboard lifecycle and artifact root, so no separate project or port is created. The one-shot component is `com.codex.ashare-dashboard.external-context-research`; it owns only public external-context files, state and logs and does not open the strategy database.
- The runner resumes the frozen `9,027`-task plan from immutable input/result checkpoints in batches of at most `100`, preserves the one-second per-request gate and two bounded task-level transient attempts, holds a single-instance file lock, uses `caffeinate -i` while active and relies on launchd for restart after a bounded zero-progress exit.
- Completion automatically regenerates the active relevance overlay and verifies every Raw/Silver/PIT manifest offline. It does not bypass the global-market gate or start a weight scan when a qualified export with provider revision lineage and replay rights is absent. V3, paper tracking and live selection remain unchanged.

[2026-08-07T01:18:00+08:00] The full CNINFO background component is installed, published and producing durable checkpoints:

- Source commit `2079df1` was fast-forwarded to `main`, pushed to `origin/main`, published to the local runtime and verified by `44/44` deploy checks. The release manifest is `/Users/hernando_zhao/codex/runtime/projects/ashare-dashboard/output/releases/20260806T170648Z-2079df1f856b/manifest.json`.
- LaunchAgent `com.codex.ashare-dashboard.external-context-research` runs the exact descriptive wrapper, uses the shared managed Python environment and keeps `caffeinate -i` active. The initial launch exposed a runtime-local virtualenv assumption before any data write; the environment was corrected to the managed source virtualenv and the subsequent launch remained active.
- The first real batch advanced the frozen plan from `150` to `250/9,027` tasks. It processed `100/100` tasks with `0` failures, `0` transient retries and `426` rate-gated HTTP requests, then atomically wrote `operations/full713-background-state.json` and entered cycle 2 without manual intervention.
- This proves installation, checkpointing and forward progress, not full713 readiness. Acquisition continues in restart-safe batches. V3 remains unchanged and the qualified-global-market gate remains mandatory before any external-weight backtest.

[2026-08-07T01:28:00+08:00] Public global-market sources do not clear the frozen replay contract:

- FRED/ALFRED has excellent vintage-date semantics, but its current API terms prohibit use in connection with storing, caching or archiving content and prohibit API access in connection with software or machine-learning systems. HKEX likewise prohibits programmatic/scripted access, systematic retrieval and automated analytical use without permission; Hang Seng index information is separately licensed.
- Cboe's public official history is useful for VIX and U.S. exchange-volume diagnostics only. It does not cover the frozen global basket, while broader historical index data is a licensed DataShop product. Stooq is rejected because authoritative terms, provider revisions and frozen-replay rights could not be established.
- No qualified vendor credential or export exists in the local environment or the user's Downloads, Documents or Desktop. Wind remains the only surviving full-basket path, contingent on entitlement, revision semantics and offline replay rights. Public accessibility will not be substituted for those gates.

[2026-08-07T22:10:00+08:00] Official-facts static-weight ablation retains V3 at lambda zero:

- The background acquisition completed all `9,027/9,027` CNINFO tasks for `3,009` symbols with no partial symbols. Active v9 curation retains `155,132/155,518` official event packages; the final offline bundle `external-context-ablation-bundle-3c3dc25afff6a04de8b0e44e` verified `467,244` files with no network. The user accepted the approximately 2.0 GiB physical footprint without compression.
- A preregistered offline runner replayed the frozen R14 `trial-000` scorer over the 1,990,456-row PIT feature matrix. Lambda zero reproduced all `511/511` non-cash signal days and every symbol/rank pair with zero mismatches. The official-facts comparison window contains `493` model signal days through 2026-05-26; this is inside the full713 data envelope but is not mislabeled as 713 model signals because the frozen feature matrix begins on 2023-09-07.
- Personal eligibility is applied before scoring using decision-day unadjusted closes and the conservative `new_retail_cash_account` profile. The `<=200 CNY` rule excluded `4,985` candidate-date rows and changed the frozen Top3/replacement outcome on nine dates. Current names and status were not backfilled into history; historical ST/risk-warning status remains explicitly unverified.
- The announcement feature uses title/metadata only, conservatively excludes same-day end-of-day records after the `23:59:59+08:00` cutoff, decays adverse events over 20 calendar days, and residualizes them with an expanding ridge model that predicts each date before updating with that date. The scan found `7,292` deterministic adverse events across `1,464` symbols and zero future-event violations.
- Static weights `0, 2.5%, 5%, 7.5%, 10%, 15%` all passed the preregistered mechanical/non-degradation gates. The 5% point had the highest tuning mean (`0.04437` versus lambda-zero `0.04199`) and changed 100 date-rank slots, but all six candidates were within one standard error. On the untouched last-year holdout, lambda zero was also better than 5% (`0.05362` versus `0.05185`). The one-standard-error rule therefore selects the smallest stable weight: `lambda=0`.
- This is signal-level overlapping-return research, not account-level capital replay and not production evidence. V3, paper tracking and live candidates remain unchanged. Rule-dynamic and learned gates do not advance until a new independent channel or a materially stronger official signal beats lambda zero on untouched data; global-market and professional-news layers remain separately blocked by qualified-feed requirements.

[2026-08-08T01:40:00+08:00] Multi-round account replay retains V3 and rejects every external-information challenger:

- All formal account-level rounds preserve `lambda=0` and compare total return, annualized return, maximum drawdown, negative-month count, worst month, skipped-order rate, skipped-signal rate and maximum exposure. No external-information candidate is non-degrading on all eight metrics while also clearing the frozen standout gate.
- The closest challenger, `external_pair_q060_transfer5`, improves tuning, validation and the reused extended readout by about `2.16`, `1.17` and `0.50` percentage points respectively, but its extended relative lift is only about `1.03%`, not the required `10%`; skipped-order rate, skipped-signal rate and maximum exposure also worsen. It remains research-only.
...
## 2026-08-09 — The 13 runtime-integration failures were infrastructure and fixture regressions, not a V3 result

- Keep the personal eligibility contract unchanged: only eligible A-share main-board names at an unadjusted decision-time price no greater than 200 CNY may be positive controls. Tests that need a tradable positive case use an affordable main-board fixture; tests for rejection retain the expensive and restricted-board fixtures.
- Runtime settings and operations views are root-only. Root-domain members cannot access research operations or manual research, while the legacy beta analyst capability remains limited to creating, reading, executing, and retrying research requests; governance completion/failure remains root-only.
