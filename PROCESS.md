# PROCESS

本文件只记录可复用的反回归原则和工程判断，不记录单次流水账。项目状态、当前阶段和下一步见 `PROJECT_STATUS.json`；发布 manifest、截图、测试输出保留在 git 历史、release 目录或对应 artifact 中。

## 维护规则

- **只写会二次踩坑的经验**：如果一条记录只是“做了什么、提交号、截图、测试通过”，不要写进这里；只有当它抽象出未来应遵守的判断、边界、排查顺序或失败模式时才写。
- **每条记录必须可执行**：优先使用“触发场景 → 原则 → 必做动作/禁止动作”的写法。避免“已修复/已发布/已验证”这类完成态描述。
- **不要把短期事实写成长期原则**：股票代码、run id、commit、截图路径、单次收益、一次性回填结果不进 PROCESS；这些属于状态文件或研究 artifact。
- **保留边界，不保留噪声**：当某次事故只贡献一个新边界条件，把边界条件归入对应主题；删除复盘中的路径、命令输出和重复验证。
- **新增条目前先查重**：同类原则已存在时，修改原条目而不是追加日期流水。
- **状态与原则分离**：`PROJECT_STATUS.json` 记录“现在到哪了”；`DECISIONS.md` 记录“为什么选择这个方向”；`PROCESS.md` 记录“以后遇到同类问题怎么不再错”。

## 完成定义

- **live-facing 任务不能停在 repo**：用户可见修复的完成链路是 repo 改动 → 本地测试/构建 → 发布到 runtime → 真实 served 页面或 API 验收。缺发布或缺真实验收时，只能说“代码已改”，不能说完成。
- **发布必须对应干净源提交**：发布前确保工作树可解释；若文档/状态在发布后又变化，需要再次发布或明确这些变化不影响 runtime。不要让 runtime manifest 长期落后于最终收口提交。
- **浏览器验收要验证真实问题点**：不要只验证页面能打开。移动端问题用移动视口检查布局指标；数据展示问题用 served 页面文本/API 同时确认；canonical 问题优先核对认证、隧道和远端路由。
- **发布刷新不能掩盖修复边界**：代码展示层修复可用 `ASHARE_PUBLISH_REFRESH_MODE=skip`，但数据链路或 run 内容修复必须补相应 runtime DB 投影/刷新，并验证实际接口返回。
- **长任务不留悬挂进程**：批量回放、浏览器自动化、短投验证、刷新脚本结束后检查并清理相关进程和 lock；不要误杀用户普通浏览器。

## AI 开发流程

- **平台本体不能落进被纳管项目**：任何“中台、自动化平台、任务调度、多 agent 编排、LLM reviewer、CI/CD 门禁、跨项目巡检、工作台总览”类需求，开工前必须先做边界判定：`platform_core`、`managed_project`、`integration_adapter`。`platform_core` 只能进入平台系统/仓库；本仓库最多承接股票看板自己的业务适配、fixture、验收样本或只读对接点。
- **超大文件豁免必须随治理收缩**：超过 500 行的文件不能因为填写了理由就长期视为安全。每次治理要优先拆出低耦合 helper、测试支撑或子测试文件；`.largefile-manifest.json` 只保留当前仍超线的真实文件，低于阈值、已删除或已拆分完成的条目必须移除，仍明显过大的文件应标为 `planned_refactor` 而不是永久 `accepted`。
- **试验田不是产品宿主**：可以用本项目验证流程，例如 Context Pack、子进程执行、评审、重跑和复盘，但输出必须沉淀为流程合同、评估文档或外部平台接口约束。不得因为试验在本项目发生，就把平台工作台、平台 projection、平台 scheduler 状态或平台治理 UI 写成本项目运行时代码。
- **边界门禁先于 Context Pack**：派发子进程前，主进程要在 Context Pack 中写明目标宿主、非目标、禁止落点和允许写入范围。若子进程输出包含未授权的 UI/API/CLI/runtime artifact，主进程必须判为越界并重跑，而不是在后续总结中解释其合理性。
- **流程失败要回滚产品面再固化规则**：一旦发现平台能力误落入业务项目，先用非破坏性 revert 撤掉产品代码，再把原因升级为 `PROJECT_RULES.md`、`PROCESS.md`、`DECISIONS.md` 或合同门禁。不能在错误宿主上继续补设计来“修正”平台能力。
- **全局协议先于并行模块**：多 agent 设计或实现前，先由主进程产出术语、事件、artifact、接口和成熟度 registry；依赖该 registry 的模块任务不能与 registry 任务同轮并行，否则容易出现模块自造事件、接口漂移和后续拼接失败。
- **子进程输入要压缩成 Context Pack**：不要让每个子进程重复读取长状态文件和全部历史合同。主进程先冻结目标、非目标、owned files、registry allowlist、成熟度限制、必读文件和禁止动作，再派发短输入；需要长文档时只给路径和具体读取范围。
- **重跑必须修复失败面而不是补解释**：评审发现未注册接口、成熟度越界、开放问题未分类或子进程越权时，进入下一轮重跑，把评审结论升级为硬约束；不能只在主进程总结里解释为什么问题“可以接受”。
- **设计阶段也要有可机检门禁**：Markdown 合同可以作为早期草案，但至少要提供 registry appendix 或 allowlist，支持脚本检查未注册事件、artifact、interface 和废弃 id；进入代码实现前再决定是否升级为 JSON Schema、DB registry 或代码生成清单。
- **实现阶段先跑 contract registry check**：当任务引用自运行流程事件、artifact family、interface、maturity 或 claim ceiling 时，先运行 `contract-registry-check` 验证 JSON registry；未注册、deprecated、`proposed_*` 冒充正式依赖时必须重跑设计或更新 registry，不能进入代码实现。

## 运行态与发布

- **runtime DB 是 live 真值源**：repo 本地库、样本库、output artifact 只能用于复现和研究；对用户解释当前页面时以 runtime DB/API 为准。
- **runtime DB 备份要有保留上限**：live 数据手术前必须备份，但大 SQLite 备份不能无限留在 `data/`。备份文件应匹配 `ashare_dashboard.before-*.db` 或 `ashare_dashboard.db.bak-*`，保留最近少量原始副本，其余由 `scripts/prune-runtime-db-backups.sh` 压缩进 `~/Library/Logs/codex-archive/ashare-dashboard-db-backups` 并按天数淘汰；当前 `ashare_dashboard.db` 永远不进入清理范围。
- **artifact 写入保护要识别项目级 `.git`**：开发仓里的 `data/artifacts` 仍属于源码污染风险，必须拒绝刷新脚本直接写入；同步后的 runtime 目录没有项目级 `.git`，其中的 `data/artifacts` 是 live 数据目录，不能被同一保护误判为 source checkout。
- **scheduled run 成功写候选后必须幂等**：短投试验田这类先写候选、后跑维护/验证的调度任务，retry 不能只依赖外层 slot 文件。写入侧要按业务日期、信息模式和触发源复用已完成 run；展示侧仍要保留语义去重，避免历史重复写入继续污染纸面跟踪。
- **runtime DB 数据手术必须先备份再做可审计最小删除**：清理 live 重复候选时，先复制完整 sqlite 库到 `data/backups`，再用可复查的候选范围删除验证快照和候选；不要为了消除看板重复而删除仍可能有独立研究含义的 LLM run 内容。
- **短投重复清理要同时修写入和读取**：重复候选不能只做一次性 runtime 清理；写入侧要幂等，读取侧要语义去重，并补专项回归。运行库清理事实属于 `DECISIONS.md` 或 release evidence，不进入 PROCESS。
- **纸面退出轨道要按真实触发时点建模**：止盈/止损类轨道不能被机械持有窗口约束。若策略定义为 10 日内随时触发，就应从买入后第 1 个交易日开始用可用的高低价检查；前端也要展示所有退出轨道，不能只把机械5日当成唯一结果。
- **canonical 不可用先分层排查**：先查 localhost 5173/8000 健康，再查 canonical 是否 302 到登录页，再查 tunnel/remote port ownership；不要先误判为代码失效。
- **canonical stale 多半是路由或隧道问题**：页面旧、接口新、资源健康时，优先检查 tunnel、远端端口和缓存态，再决定是否重新发布。
- **发布重启服务不能用旧 `launchctl unload/load`**：触发场景——canonical 报 `connect ECONNREFUSED 127.0.0.1:5173`，但本地前端服务随后又恢复。原则——发布脚本重启 frontend/backend 应用 `bootout/bootstrap` 控制 GUI domain，并给 backend 慢启动留足健康等待窗口；旧 `unload/load` 容易和 KeepAlive 抢端口，制造 `EADDRINUSE` 和公网短暂空窗。排查顺序——先看 `5173/8000` 本地 health，再看 frontend LaunchAgent stderr 是否有 `EADDRINUSE`，最后查远端 `3102/4101` tunnel 端口是否真实监听。
- **mounted app API fallback 不能越过挂载前缀**：canonical 路由挂在 `/projects/ashare-dashboard/` 时，前端请求只能优先走同前缀 `/projects/ashare-dashboard/api/...`。不要把超时或 HTML fallback 降级到站点根 `/api/...` 或无 `/api` 的 SPA 路径，否则会制造 404、误报面板失败，并掩盖真实 API 慢查询。
- **LaunchAgent 要区分任务和服务**：定时任务用 `RunAtLoad` + `StartCalendarInterval` 或 `StartInterval`；服务进程用 `RunAtLoad` + `KeepAlive`。需要精确时钟触发时不能只靠 `StartInterval=300`；关键时点应配置显式 calendar trigger，并让任务用 slot state 保证多次唤醒不重复写入。
- **SSH 隧道要自愈且能清旧占用**：重连失败先查远端旧 sshd 是否占端口；清理脚本改动后跑语法检查；隧道进程遇错应退避重试。
- **SQLite 写锁要按工作流避让**：批量短投验证、历史回放、盘后刷新和页面验收不要无脑并发；遇到等待时先判断锁持有者，不要把慢写误判为 Python 崩溃。
- **后台热路径要避让定时刷新锁**：API 后台 tick、定时刷新和维护补算都可能写同一个 runtime SQLite。服务型后台写入在执行前必须识别 scheduled refresh 的 `run.lock`，锁活跃时跳过本轮，而不是和盘后/午间刷新争写锁。
- **维护型短投补算不能挂在前台热路径**：recent validation catch-up、failed-round retry、长窗口回测和行业归因补数都属于维护任务；默认不要接在 live daily shortpick slot 后继续占 SQLite 写锁。需要补跑时用显式开关或维护窗口，并在前后检查前端小接口健康。
- **前端统计读数要走投影表**：首屏统计、聚合历史结论、运营摘要和模型反馈趋势应由定时/触发式任务写入 `frontend_projections`，API 只读小 payload。页面请求不得临时扫大表、读整包 artifact、跑统计或写库。
- **大 universe 回测必须先扁平化数据**：3000 只股票级别的历史回测不要在循环里反复构造 ORM 对象或重复扫描全市场基准；先把日线、特征、候选池和基准路径变成可复用的扁平缓存，再做小网格或精确确认。

## 策略与研究

- **训练隔离必须落实到标签可知日期**：20个交易日不能只写成split元数据或20个自然日。先计算全部依赖（个股、基准、跨持有期共享状态）的最晚形成日期，再以完整横截面过滤训练日期，最后检查最少训练样本；必须用“改未来标签不影响过去拟合”的反例验证内存与流式路径。
- **缺失持有期目标不得借用其他持有期**：5日/20日目标缺失时应阻断；10日收益不能默认为其他持有期标签。修复后对旧报告增加重验边界，不能用重新生成的旧历史冒充独立样本外。
- **信号日订单须冻结股数与现金预留**：未来收盘价、未来卖出回款不能决定过去计划的购买数量；现金账必须同时包含可用现金、已预留现金和持仓市值，缺报价和无法核实的成交单独记账。

- **执行覆盖不是默认硬门禁**：当产品目标明确为稳定盈利且允许空仓时，低置信度替补带来的跳过率改善不能覆盖其负损益贡献。应把跳过率作为显式权衡而不是一票否决项，并用新配置 ID 固化目标函数变化；影子观测与真实下单权限必须分离。
- **策略发现先看特征种子再写公式**：当目标是“模型找到策略”时，先用 point-in-time feature matrix + executable label matrix 做特征/方向/horizon 诊断，确认是否存在正 top-bucket 收益种子；没有种子时扩展数据来源和特征族，不要继续调 momentum/turnover/volatility 小公式或把相对少亏包装成策略。
- **冻结策略不等于跳过治理**：池子大小、TopK、阈值、惩罚系数、退出窗口、入场口径和对照版本都必须进入 policy config，并由代码投影使用；policy audit 要覆盖新策略域。
- **冻结主线只能按原证据口径前向观察**：已冻结的主策略不要因为一两天纸面表现改参数。新想法用并行对照组验证，不能直接替换主线。
- **入场口径变更是新研究，不是小修**：次日收盘、次日开盘、涨停可成交假设会改变历史证据；必须并行记录 entry price source，并让页面展示清楚。
- **盘中限时对照不要跑完整研究链**：如果用户要求固定时点前出结果，先判断耗时上界；能准点的只能是确定性规则 + 快照数据，LLM daily-analysis 和大规模补数继续留在盘后批次。
- **涨停可执行性要保守标注**：一字涨停不能假设买入；开盘入场研究遇到接近涨停开盘也不能假设成交。普通收盘涨停如果日线无法证明无成交，应在口径说明里暴露不确定性。
- **账户可买范围先于选股优先级**：严格纸面对照和可执行研究必须先过 `new_retail_cash_account` 过滤；创业板、科创板、北交所、ST/退市风险标的可保留为研究样本，但不能进入新开户普通现金账户严格交易对照。
- **对照组要回答单一问题**：前三等权回答单票依赖，金叉回答趋势确认，非涨停追高回答可执行性，开盘版回答入场时点。不要把多个假设混进一条对照。
- **候选验证与资金曲线不能混排**：逐候选短窗口均值回答“选股池平均质量”；组合资金曲线回答“按资金规则执行后的账户路径”。展示和结论必须分口径。
- **指数/基准缺失不能显示成 0**：缺三大指数或同行基准时显示缺失/不可用，用等权市场基准或明确的 fallback；不要让空序列在 UI 上像真实 0%。
- **小样本均值不能包装成稳定策略**：如果收益来自少数强尾部或中位数为负，应明确“观察性研究”，不能提升为生产级证明。
- **案例可分不等于机制可泛化**：由少数热点股启发的状态规则必须先冻结，再在更宽的 PIT 样本上回放。即使规则能排除案例里的假反转并命中随后上涨，也不能覆盖宽样本胜率、账户收益或对照门禁失败；失败后应更换特征族或模型目标，不能围绕案例继续调阈值。
- **学习排序与概率门控要分开验收**：当 expanding 模型出现零触发或覆盖骤降时，必须分别报告每日 TopN 的样本外排序表现、概率校准漂移和最终账户门控结果。排序均值为正不能掩盖低胜率/尾部依赖，概率门槛失效也不能在同一复用区间事后放宽；校准方案必须在新独立区间前重新预注册。
- **优化候选必须先校准基线**：任何更快的回测器、向量化筛选器或近似模拟器，必须先复现当前已确认策略在 canonical artifact 中的核心结果，再用来替换冻结线。不能用一个尚未校准的快工具证明“新策略更好”。
- **冻结可以分级但不能混名**：当一条规则是“当前最佳已确认纸面候选”但未通过生产门禁时，页面和结论都要保留这个降级语义；不要把纸面观察、研究候选、生产证明三个等级混成一个“冻结策略”。
- **盘中 SLA 要拆成外部拉数和本地计算**：14点这类固定时点任务必须分别记录行情源耗时、粗筛耗时和选股耗时。本地计算达标不代表整条链路达标，实时行情源超时要按独立风险治理。

## Short Pick Lab

- **纸面跟踪 API 必须只读**：打开试验田、纸面跟踪或历史回放不能触发行情同步、模型调用、DB 写入或全量市场研究。
- **固定日期测试要隔离当前日期漂移**：短投验证测试如果使用固定行情窗口和固定 run_date，`validate_recent_*` 的 lookback 必须动态覆盖该 fixture 日期，或显式冻结当前时间；不要让真实日期推进导致回归测试从“刷新失败”变成“没有 recent run”。
- **历史回放页面只能读预计算统计**：feedback、策略收口和组合回测摘要必须由离线脚本或已完成 artifact 生成；页面请求不能临时扫 validation 表或跑 market-factor study。
- **当前/下轮语义必须看 signal date 和 entry date**：已入场记录、下轮选择、当前交易中选择要分开展示。不要用最新 run 覆盖已入场 ledger。
- **历史 seed 要显式写 signal/entry date**：冻结前已有人工盯盘名单时，回填必须带原信号日和实际入场日，避免页面显示“没有纸面追踪”。
- **LLM 纸面对照只能从可买池里固定一只**：全量 LLM 候选保留研究用途；进入严格纸面对照的唯一候选必须先过账户可交易性过滤，再按固定排序提前标记。
- **市场因子 overlay 删除重建要局部化**：重新投影某个 run 时只删除该 run 的 market-factor overlay 和关联验证，不影响 LLM 原始候选和其他运行记录。
- **无 live run 也要能看历史回放**：试验田 tab 的可见性不能只挂在 latest live run 上；只有 replay 数据时也应显示历史回放。
- **短投列表接口要轻量**：run 列表不嵌 full candidates、source packet、raw answer；大 payload 走下钻接口，避免打开试验田时解析数 MB JSON。
- **历史回放聚合接口要投影后再下发**：长窗口策略证据可以来自重 artifact，但 `/shortpick-lab/replay-feedback` 只应返回首屏和当前下钻需要的 rows；全量 period/regime/time-slice 明细留在 artifact 或专门下钻接口里。
- **历史分析结论只做 projection**：首屏决策读数、可执行性漏斗、入场矩阵必须由已完成 artifact 或只读 ledger 投影；缺字段显示待补原因，不在前端拼结论，也不在页面请求里补跑研究任务。
- **长窗口策略证据要和 LLM replay 分名**：当 LLM 历史 replay 时间窗过短时，可以用 full-window staged portfolio artifact 扩充确定性策略族的跨时段和分行情证据，但页面必须明确它回答的是组合路径稳定性，不等同于扩充了 LLM 自由选股样本。
- **非 LLM 高级分析也要声明数据粒度**：组合级置信区间、稳定性和归因可以从 staged portfolio 月度/季度/行情桶产物投影；股票/行业归因必须等全量逐笔 artifact，不能用 `trades_sample` 外推。
- **纸面退出结果要默认浮出**：机械 5 日这类已完成退出不能只藏在搜索和翻页里；试验田应有退出数量、等待窗口、最新退出和退出状态筛选。落实见 `8ef154e Surface shortpick paper tracking exits`。
- **纸面验证补算要覆盖到期旧批次**：机械 10 日、20 日这类长窗口到期时，旧 run 可能已被每日新 run 挤出“最新 N 个批次”。定时补算必须额外纳入近窗口内仍有 pending validation 的批次，否则页面会长期只显示 5 日退出。

## 历史回放与证据隔离

- **历史回放必须 point-in-time**：prompt 只能包含 sealed source packet 和当日可交易 universe；不得调用现时搜索、现时行情补数或未来 exit bars。
- **诊断代理不能冒充真实 LLM**：proxy 只能显式标为 diagnostic；真实 LLM 失败就记录失败 round，不自动生成可混入正式统计的 proxy 候选。
- **来源引用失败要 fail closed**：候选引用 rejected source、packet 外 source id、cutoff 后来源或未验证时间来源时，应保留原始引用并触发 leakage audit failure，不要静默替换成看似合法来源。
- **unsupported claim 不等于未来泄漏**：严格来源样本和可交易验证样本要拆开。来源证据不足可以退出 strict 样本，但只要无硬泄漏且账户可买，可进入可交易口径观察。
- **历史回放首屏先回答研究问题**：先展示覆盖日期、正式样本、周期表现、对照组和统计门槛；packet/source/泄漏审计放下钻区。
- **历史回放样本主体应是 candidates**：不要只依赖单个 primary pick；模型输出预算和 repair 逻辑要支持候选列表，避免样本数不足。

## 多账号与权限

- **先分清真串号和展示误导**：多账号问题先用 API 验证账号态和数据归属，再判断是后端隔离失败还是前端把全局候选池混进账号工作台。
- **`act_as` 不跨刷新持久化**：root 代看是单页临时状态；刷新或重开标签默认回 actor 自己空间，避免用户误以为持仓消失。
- **member 初始化顺序不能复用 root**：先取 `/auth/context`；root 可走 runtime settings，member 应走 overview/只读入口，避免 settings 403 白屏。
- **个人模型 Key 必须按账号隔离**：人工研究和追问是所有账号的工作流，不应因为 member 不是 root 而隐藏；但外部模型 API key 必须按 `target_login` 读写、默认和 failover，root 代看时也只能维护被代看账号自己的 Key。全站数据源 token 仍按运维配置处理，不混入个人模型配置。
- **关注池隔离不等于分析断流**：账号 follow 分开存；symbol 级分析资格按所有账号 active follows 并集判断。只有最后一个 follow 消失，才停止全局 tracking。
- **空成员空间不能启动空模拟盘**：member 首登可以有 draft session，但 `can_start` 和 API 都必须拒绝空 watch symbols。

## 前端与移动端

- **破坏性轻量操作不能被后续全量刷新反向判失败**：关注池移除、状态标记这类 mutation 只要服务端已确认成功，前端应先更新本地可见状态，再后台刷新 shell/projection。后台刷新失败只能提示“已操作但刷新失败”，不能把成功 mutation 包装成删除/标记失败；请求超时文案也必须区分单次连接等待和总超时预算，避免 3 秒中止显示成 10 秒总超时。
- **移动端不要压缩 PC 工作台**：`App.tsx` 保持数据和 handler 入口，手机端使用 `components/mobile/*` 独立 shell；宽表和 PC tab 不应直接塞进移动端。
- **移动端交互要有明确 affordance**：只读行不显示右箭头；二态偏好用 Switch；多选/模型选择才进入二级菜单。
- **破坏性滑动操作只在展开态显红**：闭合态不要透出红色；展开态处理相邻圆角；释放后吞掉下一次 click，防误开详情。
- **底部导航内容按整体居中**：按钮不要用容易受字体和安全区影响的两行 grid；图标和文字作为一个整体垂直居中，并固定 line-height/尺寸。
- **加载态和空态要分清**：接口未完成时显示加载中/骨架；确认无数据才显示暂无。不要用 `-` 让用户分不清是空还是慢。
- **页面应先回答用户问题**：历史回放、策略收口、复盘页首屏放结论性统计和操作入口；技术审计、source packet、raw payload 放下钻。
- **用户可见文案禁止内部指令味**：避免 `must/builtin/verified` 这类实现词；用“探索结果 / 默认模型 / 短窗口候选统计 / 长样本账户路径回测”等业务词。

## 数据与分析流水线

- **时间窗口覆盖不能冒充模型信号日数**：外部数据可以覆盖 full713 时间包络，但冻结特征矩阵可能因 warmup、现金开关或源矩阵起点只产生更少的真实选股日。报告必须同时列出数据包络、矩阵交易日、非现金信号日和实际消融日；`lambda=0` 要逐日复现 symbol/rank，而不是只核对总行数。
- **外部信号消融先保留核心策略的内部阈值语义**：外部公式可用 `z(core)` 排序，但现有选股器中的 score 阈值、替补和仓位规则仍应读取原始 core score。实现上使用独立 ranking score，避免 `lambda=0` 因标准化而悄悄改变原策略。

- **外部数据 fail-open**：AKShare、公告、实时信息流等外部接口可能超时、空表或异常；刷新 pipeline 中这类失败应降级为空数据/告警，不应中断 intraday sync、simulation step 或其他主流程。
- **基础主数据空表要走缓存兜底**：短投全市场 universe 依赖 `stock_basic` 这类低频主数据时，外部接口短时空返回不能直接阻断当日纸面跟踪。先按同一账户可买规则使用本地 `stocks` 缓存继续生成 universe；只有缓存规模低于完整 universe 下限时才 fail closed，并把 fallback 来源写入刷新 summary。
- **事件分析接入不能阻断主刷新**：新增事件检测、LLM 分析、资料抓取时必须包裹异常，避免一个新模块拖垮盘后全流程。
- **simulation 刷新顺序固定**：不能只 restart session；需要 `restart → step → rebuild`。持续运行用交易时段后台 tick + 单次 anchored step，不复用研究刷新链路。
- **Phase 5 artifact 查找要容忍 legacy 字段漂移**：payload 中 `backtest_artifact_id` 等旧字段可能不稳定，优先解析真实存在 artifact。
- **盘后 freshness 文案用快照时间**：用户可见层显示“截至 MM/DD HH:MM”，不要暴露原始秒数或内部 freshness 算法。

## API、脚本与工具陷阱

- **API 错误响应统一用 `detail`**：前端只提取 `payload.detail` 展示中文错误；返回 `error` 会退化成裸 HTTP 状态码。
- **不要覆盖仓库默认 pytest marker 策略**：跑完整默认门禁时使用 `PYTHONPATH=src python3 -m pytest -q`，让 `pyproject.toml` 的 `not slow_integration and not runtime_integration` 生效。临时 `-m "not integration"` 会覆盖默认排除，把刻意隔离的慢集成/runtime 用例纳入，导致与默认门禁不一致的失败信号。
- **Prompt 文案改动要同步断言**：用户可见 prompt 或 follow-up copy 改词后，跑相关 view/prompt 测试，避免断言继续锁旧词。
- **Pydantic 前向引用不要藏在 `TYPE_CHECKING`**：Pydantic v2 用模块命名空间解析注解；循环依赖时在模块加载完成后注入类型并 `model_rebuild()`。
- **bash 命令拼接不要 `.join('; ')`**：多余分号会制造 `do;`、`then;` 语法错误。生成脚本后跑 `bash -n`。
- **自动化浏览器清理要精准**：清理 Playwright daemon 和临时 profile，不要 `pkill Chrome` 误伤用户会话。
- **文件批量替换要保留行结构**：用脚本改文件时新内容必须按行插入，避免把函数体压成一行；复杂编辑优先用小 patch 或格式化工具验证。
- **运行时 SQLite 必须 WAL，长写任务不能阻塞看板读**：触发场景——后端与盘后日刷两进程同连一个 db 文件，日刷是 ~50min 全量写。原则——`db.py` 连接级固定 `journal_mode=WAL + synchronous=NORMAL`，否则默认 rollback journal 写者排他会让所有读 30s 超时（“网页能开、所有 tab 超时”的典型症状）。必做——切 WAL 前备份 db，切换后重启 backend 让旧连接统一到 WAL；长流水线按步骤拆分 `session_scope`，写锁只在实际落库时短暂持有。
- **LaunchAgent 不能让 reload/publish 触发重型任务**：触发场景——调度类 agent（如盘后日刷）跑重型长任务。原则——这类 agent `RunAtLoad` 必须为 false，发布脚本不得在 bootstrap 后 `kickstart -k` 强制启动；让 StartCalendarInterval/StartInterval + 幂等 `.ok` slot 守卫决定何时跑。否则治理期间反复 reload/publish 会叠起多个重型写任务，连锁触发上一条的锁竞争。排查顺序——面板超时先查后端是否 `database is locked`，再查是否有 `phase5-daily-refresh` 在跑、它的触发源（RunAtLoad / kickstart / tick）。
- **打断重型 slot 的重试退避要 ≥ 任务时长**：被 kill 的长任务若退避窗口短于其运行时间，会在上次影响未沉淀时重试叠加。`SLOT_RETRY_INTERVAL_SECONDS` 等退避默认值应 ≥ 对应超时上限。
- **盘后验证窗口数据要在同步层一并到位**：触发场景——shortpick 验证算超额收益需个股+基准两条 K 线都到目标交易日。原则——`--analysis-only` 日刷也必须同步基准指数（不要只在 ops 刷新里同步），否则个股到最新日、基准滞后一天，5d/10d 退出永远卡在 `pending_benchmark_data`/`pending_forward_window`。配套——验证补算要做成有界循环（`max_iter` + 已处理去重 + 「本轮无新 completed 即停」），数据到位的 pending 一次补齐，真缺数据的不死循环、不反复拉。
- **手动 kill 重型验证后要 checkpoint WAL**：WAL 模式下被中断的大批量写会留下几百 MB 未 checkpoint 的 `-wal`，使后续读（如纸面追踪 ledger）变慢甚至像卡死。中断后跑一次 `PRAGMA wal_checkpoint(TRUNCATE)`，必要时重启 backend 清掉退化状态。排查"endpoint 卡死"时先分清是 ledger 计算慢（独立跑 0.6s 即排除）、auth 依赖、还是 WAL 膨胀。
- **研究大载荷必须区分在线钉住与冷归档**：完整矩阵和当前候选源可以因复用价值常驻，但必须在运行时存储政策中逐个钉住；同目录的旧矩阵、旧标签、旧 candidate-run 和完整账户回放不能因“以后可能有用”无限堆积。统一执行真实目录生命周期门禁，未钉住载荷先生成 SHA-256、单线程压缩并校验，写恢复元数据后再移出在线目录。
- **逻辑 artifact 引用必须声明是否物化**：流式重建可能只需要 input snapshot 而不生成独立 universe matrix。此时必须记录 `logical_only_not_materialized_by_streaming_rebuild`，不能留下一个看似缺失文件的裸 artifact id；历史 `code_version=unresolved_local_checkout` 只能诚实降级为冻结输入可复用，禁止补写虚假的提交版本。
- **发布成功标记前必须完成存储治理**：发布流程应先归档过期数据库备份、执行真实研究目录生命周期审计、轮转发布快照和可重建运行时缓存，最后才更新 `latest-successful`。治理失败不能留下一个看似成功的发布标记。

## 外部信息账户回放真实性原则

- **研究大快照必须放在持久 artifact root**：长时间采集结果不能依赖临时目录；每个可回放输入都要冻结摘要、内容哈希、时间覆盖和来源许可假设，历史回测只读本地快照。
- **有效区间不等于历史可见时间**：供应商返回的 `in_date/out_date` 可以消除当前静态行业回填，但不能替代当时的 `published_at/first_seen_at/revision`。数据层必须同时提供严格 `available_at` 回放和明确标注的有效期研究模式；后者不得晋级生产。
- **重叠矩阵不得让新修订覆盖旧冻结行**：增量矩阵与历史冻结矩阵重叠时，按最早冻结版本保留历史，后生成版本只记录特征与核心分数差异。即使最终分数相同，也不能静默改写输入谱系。
- **公开源批量采集要有有界重试与幂等 checkpoint**：单次批任务只重试可恢复错误，完成的 checkpoint 不再联网，失败不留下半写文件；这样中断或更新 Codex 后都能从最近 checkpoint 继续。
- **令牌型供应商的旧明文配置必须在传输层升级**：已知官方主机的 `http` 配置可以在发送请求前规范化为经证书校验的 HTTPS；令牌只发往显式允许的官方主机，其他 HTTP/HTTPS 地址都拒绝。不能因为用户授权数据研究就把授权解释为允许泄露令牌。
- **大批量结构化响应要区分传输失败和业务失败**：空响应、超时、`IncompleteRead` 只做有限退避并丢弃半截内容；权限、参数、行数封顶等业务错误不重试。长文本导致响应持续不完整时，可以降级到预注册的数值字段，但必须在 checkpoint 记录字段变体。
- **事件水平不等于事件惊喜**：利润同比区间和快报同比值即使比标题更可控，也可能已被股票路径吸收。若 level residual 经 one-SE 选择零权重，不得继续调窗口和权重；下一种表示必须是可验证的修订差或“快报相对先前预告”的 realization surprise，并使用新未见结果。
- **`lambda=0` 复现检查以经济结果为准**：冻结 NAV 与实际成交买卖账本必须完全一致。为阻断伪成交新增的 shadow skip reason 可以改变完整订单日志摘要，但不能改变成交结果；若改变，整轮结果作废。
- **候选池深度必须写入研究合同**：Top3 内换序、Top5 近似同分破平、Top20 提升是不同问题，不能用较浅池结果替代较深池结论。
- **增量 curation 必须绑定自己的 manifest scope**：多个时间计划共用 artifact root 时，不能把“本批 manifest 数”与“整个根目录 manifest 数”直接比较。curation 必须冻结 plan-scoped manifest IDs 及摘要；离线验收要分别报告“计划任务完整”和“full713 时间窗口完整”，不能用短增量计划冒充 full713，也不能因共享根目录误报计数损坏。
- **现金路径副作用不是 alpha**：候选被删除或换序后释放的现金，若让原基线未成交股票随后成交，属于 candidate-only cash-path buy；必须用冻结基线实际买入符号约束或等价 shadow replay 排除，不能计入外部信号收益。
- **加仓造成的后续缺单也不是择时 alpha**：同股同日增加预算即使提高总收益，只要共享现金使后续基线买单消失，就不能宣称状态触发有效。必须追加独立资金精确对照，保持原股票、排名、仓位、持有期和执行配置，仅改变触发日期子集；若该子集仍弱于完整基线，应关闭该触发方向。
- **稀疏子策略要同时看资金占用**：独立账户只在少数触发日交易时，绝对收益天然受现金比例影响。报告必须同时给出收益、平均投入比例和相对完整基线的结果；可以做暴露归一诊断，但生产 carrier 仍按真实闲置现金计价，不能用理论满仓收益替代。
- **反转响应按因果层次逐级排除**：先检验新股票选择，再检验同股共享现金加仓，再用独立账户复核纯时点，最后用原 TopK/原权重/原期限/原执行配置做精确归因。任一层的收益若依赖改变后续买单或改变持仓配方，就不能归因给反转信号。
- **提前退出必须同时跑共享现金账与冻结买入账**：卖出会释放现金、改变后续复投预算、整手可买性和集中度。共享账回答完整执行结果，冻结买入股票与股数的影子账回答退出本身是否有效；两本账不能同时过门禁时，不得把结果归因给退出信号。
- **止损接近持平不等于风险改善**：部分减仓可能让验证期收益看似持平，但若调参期下降、冻结买入账下降或后续买单集合改变，仍应拒绝。不得在看到结果后继续缩小减仓比例或移动阈值寻找偶然交点。
- **固定规则失败后升级模型必须增加证据，不是增加复杂度**：持仓危险率/剩余价值模型只能使用滚动的 PIT 特征和已完成旧持仓标签，必须 expanding-window 训练、次日执行、保留基线对照，并等待新的独立时段；不能在同一复用窗口上训练黑箱再宣称解决。
- **失败轮次不得事后降低门槛**：收益、回撤、月份稳定性、跳单、跳过信号和暴露按预注册口径共同判断；不得把 deliberate no-order 改名规避失败，也不得在已查看延伸段后称其为 untouched holdout。

## 已归档流水来源

历史详细发布流水、提交、截图、测试输出已经从本文件压缩为以上原则。需要追溯单次事实时查 git 历史、`PROJECT_STATUS.json`、release manifest 或对应 `output/` artifact，不再把这些细节追加回 PROCESS。

[2026-06-04T11:45:00+08:00] 股票工作台刷新位置和试验田首屏接口治理：
本轮为股票工作台补 URL-backed route 状态，默认进入 `试验田 -> 纸面跟踪`，并把试验田首屏从“加载所有 tab”改为“只加载当前 tab”。纸面跟踪完整账本仍由纸面页读取；首页徽章改用 `/shortpick-lab/paper-tracking/summary`。短投 run 列表只返回列表必要字段和失败轮次提示，完整 rounds/candidates/consensus 走详情接口。

补充说明
- 根因文档与状态表见 `docs/contracts/STOCK_WORKBENCH_ROUTER_API_GOVERNANCE_PLAN_2026-06-04.md`。
- 本地 worktree API 实测：`/shortpick-lab/paper-tracking` 从 `3.855s` 降至约 `0.298s`；`/shortpick-lab/runs?limit=100` 从约 `1.9MB/4.338s` 降至 `113KB/0.132s`；并发试验田读路径墙钟约 `0.475s`。
- `operations/summary` 的 `27s` 症状被定位为 projection key miss：已有 projection 不覆盖当前 active symbol；fallback 构建后现在会写回对应 `operations_summary` projection。`002475.SZ` 首次 miss 约 `1.630s`，第二次同 key projection hit 约 `0.003s`。
- 本轮已发布并完成 runtime/canonical 验证：`54c9fd7374cd` 已同步到 runtime，release verifier 使用 75s timeout 通过 local/canonical parity，latest-successful 指向 `output/releases/20260604T115622Z-54c9fd7374cd/manifest.json`；本地 `5173` 和 authenticated canonical 浏览器均确认裸入口进入 `试验田 -> 纸面跟踪`，刷新不退回首页。

[2026-06-04T20:31:48+08:00] 运营复盘全量端点不能作为用户或发布验证热路径：
本轮专项治理 `GET /dashboard/operations` 的 `82.21s / 4.7MB` 超时问题。根因不是单一 SQL 慢，而是读请求混合维护 tick、完整 dashboard 聚合、深层响应校验/序列化和每组合 1250 点 `nav_history` 的大 payload。方案和代码均已由 DeepSeek 复核，必须修复项已处理。

补充说明
- 方案与状态文档见 `docs/contracts/OPERATIONS_DASHBOARD_PERFORMANCE_GOVERNANCE_PLAN_2026-06-04.md`。
- `/dashboard/operations` 兼容端点改为只读，不再在 GET 中执行 `run_operations_tick(session)`，并关闭全量 response model 校验。
- operations portfolio 响应对 `nav_history` 固定限量 90 点，同时保留首尾、净值/基准峰谷、最大回撤和最大暴露，再等距补点，避免趋势图丢关键锚点。
- release verifier 不再请求全量 `/dashboard/operations`，改为 summary + all bounded operation details，并把所有 section 合并后做用户可见文本审计。
- worktree HTTP 实测：summary `0.009s/41KB`，portfolios detail `1.44-1.61s/449KB`，legacy `/dashboard/operations` `1.96-2.17s/946KB`。

[2026-06-04T21:47:23+08:00] 运营复盘 `19.6s` 发布阻塞已按治理流程收口：
用户明确拒绝接受发布验证中 legacy operations 约 `19.6s` 的残余慢路径后，本轮继续按 `worktree -> 改进方案 -> DeepSeek 方案审核 -> 落地 -> DeepSeek 代码审核 -> 推送合入 -> 删除 worktree` 流程治理。追加根因是非组合 bounded details 仍复用完整 dashboard 构建，以及 operations cache prewarm 曾同步阻塞 ASGI startup。已拆分 section-specific details builder，并把 operations response prewarm 改成后台 best-effort。

补充说明
- DeepSeek 已复核追加方案、Step 10 代码和 Step 11 后台 prewarm 代码，结论均为无阻塞合入问题。
- `origin/main` 已包含运行时代码提交，发布 manifest 为 `output/releases/20260604T134157Z-2cd4b24213b4/manifest.json`，deploy verifier `19 passed, 0 failed`。
- runtime 验收：portfolios detail `0.007s/476KB`，summary `0.034s/43KB`，legacy `/dashboard/operations` `0.011s/556KB` 且 `simulation_workspace=null`。首次 post-publish replay 曾因启动/后台预热竞争抖到 `18.578s`，连续复测恢复为 `0.287s`、`0.194s`、`0.119s`。
- 浏览器验收：本地裸入口和显式 `?view=shortpick&shortpickTab=paper-tracking` 均进入 `试验田 -> 纸面跟踪`，刷新后不退回首页，控制台无错误。
## 2026-08-08 — Rebase partial boundary months inside each evaluation segment

When tuning, validation, or final evaluation starts mid-month, never reuse the account's whole-calendar-month return for that boundary month. Recompute monthly returns from the segment starting NAV, and attribute order metrics by trade day. Otherwise adjacent evaluation segments overlap and a reused boundary month can silently contaminate non-degradation gates.

## 2026-08-08 — External holding modifiers must freeze downstream core order identity and size

An external exit-horizon change can increase or decrease account NAV and thereby alter later board-lot sizing even when later symbols and ranks remain identical. That path amplification is not independent external alpha and can worsen forward concentration. For holding-modifier research, freeze the baseline's executed buy symbols, ranks and share counts; treat any candidate-only symbol, missing baseline buy, or changed share count as a failed authenticity check.

Core-first residual gating is also mandatory. If a candidate works only when external features overturn a below-median stock-only core signal, test a core veto before optimizing fractional weights. Report trigger count and leave-one-trigger-out results separately from aggregate gates; an all-metric pass driven by one stock is a concentration-limited challenger, not production evidence.
# Round 75 影子前向验证：历史回填必须与真实前向分账（2026-08-08）

- 一个历史候选进入前向观察时，不能把回测曲线直接续接成“真实前向”。同一策略 ID 下也必须保留 `retrospective_pit_backfill` 与 `true_forward_shadow` 两种证据标签，并明确回填不计入晋升样本。
- 影子组必须拥有独立账户状态和交易记录；它可以读取 V3 同源候选数据，但不能修改正式 V3 账户、计划单或 live order。
- 延期决策注册表采用 append-only：已评估日期只能向前推进，旧信号内容不得修改，新信号不得迟到回填到已经冻结的决策日。
- 当前生产基线与候选研究时的核心入场流不同时，前向影子必须复现候选自己的冻结入场流；不能为了接入方便改成另一条 V3 入场流，否则历史与前向不再是同一策略。
- 既有纸面账本不能用当前数据库重放后直接覆盖。若历史逐日候选源未完整固化，重放会悄然改写旧账户；首次新增对照必须保留旧组，后续改为从冻结状态 append-only 增量结算。
- 外部信号注册表过期时不能沿用最后一次结论。核心影子可以继续，但外部动作必须 fail closed，并在 API/UI 暴露冻结日期。
- 历史回填完成不等于候选成熟。Round 75 仍需至少 3 个独立真实前向延期事件，再重跑完整八指标门槛。

## 运行时回归测试的时间、权限与子进程边界（2026-08-09）

- 固定历史日期的测试若验证的是去重、分页或有界补算，应根据夹具日期计算测试专用 lookback；不要扩大生产窗口，也不要让日历推进自然制造失败。
- 账户适当性进入最外层过滤后，普通正向夹具必须同时满足板块和未复权价格资格。高价股和未开通板块只保留在明确验证拒绝原因的测试中。
- 角色迁移不能把旧 beta `analyst` 粗略映射为普通 member 后丢掉能力信息。统一认证上下文必须同时保留账户空间角色与兼容访问角色，并在端点按能力校验。
- 子进程测试必须整文件顺序运行一次。孤立用例能通过，不代表前序数据库连接和文件描述符不会污染 worker；worker 应关闭继承描述符、直接使用稳定入口，并保留有限 stderr 诊断。

## 不同起点的影子研究不能进入同一纸面追踪比较（2026-08-14）

- 同一纸面页中的正式策略和对照组必须拥有相同的起算日、初始本金和增量更新合同；否则收益曲线不可直接比较。
- 研究候选若需要历史回填才能接到更晚的真实前向起点，应留在独立研究产物中，不能通过“影子对照”身份混入纸面成交、净值和摘要统计。
- 移出纸面追踪时保留独立研究文件以便审计，但日刷、读模型和前端活动策略集合必须同时移除，避免只隐藏 UI 而后台继续运行。

## 事件型策略只能把实际触发动作的事件计入事件效果（2026-08-16）

- 候选账户可能因为更早的交易形成不同现金、持仓和净值路径。即使某个后续事件没有改变任何选股，该事件起点后的候选/基线收益也可能不同；这属于继承路径，不是该事件的捕捉能力。
- 事件胜率、事件均值和 one-SE 选择只能统计候选在该事件活动窗口内确实改变了决策的事件。检测到事件但没有动作时保留在覆盖率分母和审计明细中，不得计入效果样本。
- 验证期至少要有预注册数量的可执行事件；只有事件检测数量、没有实际动作数量，不能晋级，也不能用后续复用窗口补足。

## 热点恢复研究必须先验证机会集覆盖（2026-08-17）

- 在优化排序模型或权重前，先按 `个人资格 -> 市场部署开关 -> 机会集 -> TopK -> 可执行性` 分层归因。若指定机制案例没有进入机会集，下游模型的收益统计不能回答该研究问题。
- 复用窗口上较好的长周期均值不能掩盖目标周期失效。快速风格反转问题必须同时报告注册的短周期结果、指定案例时点、进入机会集的最早日期和当时排名。
- 每日全市场前分位股票日是覆盖率诊断分母，不是组合胜率。报告必须把 universe coverage 与账户收益、持仓数和现金开关明确分开。
- 当原策略的 TopK 身份成为硬门票时，所谓挑战者只能重排旧机会，不能发现被原策略漏掉的新热点。后续机会头应让核心分数成为软质量/风险证据，并保留个人资格和 PIT 可执行性作为真正硬过滤。

## 行情库“当前股票数足够”不等于历史截面完整（2026-08-17）

- 全库最早/最晚日期和 distinct symbol 总数不能证明每个历史交易日都有完整横截面。回测前必须抽查注册起点、分段边界和最终日的逐日股票数，以及满足最小回看窗口后的实际特征行数。
- 数据覆盖不足产生的收益结果一律标为 invalid，不得解释为模型通过或失败。只修正数据源时冻结 amendment，保持策略参数、门禁和标签不变，再从头重放。
- 热点候选“进入机会集”与“被模型正确排序”是两个独立门禁。案例覆盖通过但排名失败，说明入口方向可保留、排序模型仍应淘汰，不能把两者合并成模糊的部分成功。
- 在通用量价回归和尾部事件分类都无法稳定排序后，停止对同一复用窗口继续扫描阈值。下一步只能引入有明确经济角色、PIT 时间戳和新增信息检验的板块/事件残差，或等待新的真实前向数据。

## 策略探索必须先证明“方向新增”，再产生实现（2026-08-19）

- 提交 `0983c63` 将历次策略经验收敛为唯一强制前读总纲。新研究必须记录所读版本、既有方向重叠、实质新颖性、PIT/基线门禁和失败留存方式；只改权重、阈值、窗口或模型形式不得视为新方向。
- 失败后只把方法、关键结果、失败机制和重开条件合并进总纲；临时代码、专项测试、重复预注册和逐日产物由 Git 历史追溯，不在当前仓库形成第二套活跃知识。
