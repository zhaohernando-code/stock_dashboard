...
- **短投多 benchmark 落地口径**：实现不改表结构，继续让兼容字段 `benchmark_return / excess_return` 表示默认沪深300；新增 `validation_payload.benchmark_dimensions` 保存三维结果。模型反馈聚合也从同一 map 计算 `benchmark_metrics`，前端用收益反馈表头的 segmented control 切换展示，不能改变候选排序、验证状态或实验边界。
