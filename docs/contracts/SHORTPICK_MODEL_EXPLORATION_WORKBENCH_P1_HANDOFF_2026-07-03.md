...
| Independent model exploration mechanism | not_started | No primary universe-date feature matrix, model spec registry, candidate prediction artifact, or model comparison report exists yet. |
| P1 objective universe-date snapshot | not_started | Implement a primary `symbol` x `as_of_date` matrix from runtime DB read-only facts, not from recommendation rows. |
| P1 PIT feature matrix | not_started | Implement point-in-time feature rows for every eligible matrix row with feature lineage and missingness flags. |
| P1 label matrix | not_started | Implement executable forward labels with benchmark excess returns, costs, T+1 and price-limit constraints. |
| P1 model spec registry | not_started | Add governed model specs and feature-group definitions before any runner emits candidate results. |
...
| `walk_forward_model_candidate_run` | implemented | Candidate model predictions, train/test windows, purge/embargo metadata and split-level metrics. |
| `model_comparison_report` | implemented | Model ranking, kill reasons, OOS metrics, PBO/DSR diagnostics, winner-dependency and promotion blockers. |
...
- Artifact store accepts new artifact families and rejects unsupported families.
- Universe-date matrix does not use recommendation rows as the primary row source.
- Feature matrix rejects future data and marks missing sources.
- Label matrix blocks missing benchmark labels instead of falling back to self-benchmark.
- Model spec registry has stable ids and bounded search spaces.
