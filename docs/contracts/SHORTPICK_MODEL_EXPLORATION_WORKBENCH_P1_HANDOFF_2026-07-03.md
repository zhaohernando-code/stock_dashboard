...
| Independent model exploration mechanism | in_progress | P1 matrix foundation exists, but no model spec registry, candidate prediction artifact, or model comparison report exists yet. |
...
| `model_spec_registry` | not_started | Governed model definitions, feature groups, hyperparameters and allowed search spaces. |
