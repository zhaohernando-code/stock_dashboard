...
| Independent model exploration mechanism | research_candidate_found_blocked | P1 vertical foundation exists: matrix artifacts, governed model spec registry, registered-spec candidate run, comparison report, PBO/DSR proxy diagnostics, winner-dependency recomputation, governance decision, dashboard projection registry and offline CLI/workflow runner. 2026-07-04 found a concentrated top5 liquidity/momentum candidate that passes model-comparison diagnostics on 80-date and 160-date runtime windows, but promotion remains blocked by governance and execution gates. |
