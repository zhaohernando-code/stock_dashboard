...
class FrontendShortpickStaticTests(unittest.TestCase):
    def test_shortpick_lab_is_independent_research_surface(self) -> None:
        ...
        self.assertIn("组合 Ledger 回放对照", component_source)
        self.assertIn("combinedLedgerRows = combinedLedger?.rows ?? []", component_source)
        self.assertIn("strategyEvidenceBasisLabel(item.evidence_basis)", component_source)
        self.assertIn("后验回放行仅来自 combined_ledger artifact", component_source)
        self.assertIn("不并入主表 items", component_source)
