...
class FrontendShortpickStaticTests(unittest.TestCase):
    ...
    def test_shortpick_lab_is_independent_research_surface(self) -> None:
        ...
        self.assertIn("const displayRows = primaryRows.filter((item) => {", component_source)
        self.assertIn("<PaperTrackingEffectCharts", component_source)
        self.assertIn("rows={primaryRows}", component_source)
        self.assertNotIn("rows={displayRows}", component_source)
        self.assertNotIn("const chartRows =", component_source)
        self.assertNotIn("rows={chartRows}", component_source)
        ...
        self.assertIn("const paperTrackingRequestBehavior: RequestBehavior", api_source)
        self.assertIn("attemptTimeoutMs: 180000", api_source)
        v1_paper_api_source = api_source.split("export function getShortpickPaperTracking()", 1)[1].split(
            "export function getShortpickPaperTrackingSummary", 1
        )[0]
        self.assertIn("paperTrackingRequestBehavior", v1_paper_api_source)
        self.assertNotIn("longRunningRequestBehavior", v1_paper_api_source)
        ...
        self.assertNotIn("基于下方{chartScopeLabel}筛选", component_source)
        self.assertNotIn("chartScopeLabel", component_source)
        self.assertIn("样本 {selectedLineObservations.length}", component_source)
        self.assertNotIn("initializedDefaultStrategyRef", component_source)
        self.assertIn("PAPER_TRACKING_EFFECT_ALL_STRATEGIES", component_source)
        self.assertIn('const [selectedLineStrategy, setSelectedLineStrategy] = useState<string>(PAPER_TRACKING_EFFECT_ALL_STRATEGIES);', component_source)
        self.assertIn("selectedLineObservations", component_source)
        self.assertIn("shortpick-paper-effect-strategy-select", component_source)
        self.assertIn('{ label: "全部策略", value: PAPER_TRACKING_EFFECT_ALL_STRATEGIES }', component_source)
        self.assertIn("const byDate = new Map<string, typeof selectedLineObservations>();", component_source)
        self.assertIn("if (observation.exitTrackKey !== track.key) continue;", component_source)
        self.assertNotIn("commonSignalDate", component_source)
        self.assertNotIn('strategyLabels.includes("冻结策略") ? "冻结策略"', component_source)
        self.assertNotIn('useState<string>("冻结策略")', component_source)
        ...
        self.assertNotIn("清除图表联动筛选", component_source)
        self.assertNotIn(
            "Charts remain comparative after table linkage, so they ignore record-group and exit-result filters.",
            component_source,
        )
        self.assertNotIn("activeGroupFilter={ledgerGroupFilter}", component_source)
        self.assertNotIn("activeExitStateFilter={ledgerExitStateFilter}", component_source)
        self.assertNotIn("onClearSelection={handlePaperEffectClearSelection}", component_source)
        self.assertNotIn("handlePaperEffectSelect", component_source)
        self.assertNotIn("累计图策略", component_source)
        self.assertIn("height: 360px;", style_source)
        self.assertIn("height: 260px;", style_source)
        self.assertIn("flex: 0 0 260px;", style_source)
