...
def test_attempt_recovery_ticket_intent_output_builds_ready_intent_after_diagnostic(
    tmp_path: Path,
    monkeypatch,
    capsys,
) -> None:
    ...
    assert payload["ticket_id"].startswith("recovery-ticket-cycle-cg-cli-")
    assert payload["cycle_id"] == "cycle-cg-cli"
    assert payload["source_diagnostic_id"] == "diagnostic-cg-cli"
    assert payload["evidence_refs"] == [
        "phase5_scheduler_diagnostic:diagnostic-cg-cli",
        "phase5_scheduler_attempt_intervention_run:intervention-run-cg-cli",
    ]


def test_attempt_recovery_ticket_intent_output_handles_empty_store(tmp_path: Path, monkeypatch, capsys) -> None:
    artifact_root = tmp_path / "artifacts"
    _guard_init_database(monkeypatch)
    _install_scheduler_handler_guards(monkeypatch)
...
def test_attempt_recovery_ticket_intent_output_blocks_missing_diagnostic_id(
    tmp_path: Path,
    monkeypatch,
    capsys,
) -> None:
    ...
    _install_scheduler_handler_guards(monkeypatch)
    write_phase5_scheduler_attempt_intervention_run_artifact(
        _intervention_run(diagnostic_id=None),
        root=artifact_root,
    )
    ...
    assert payload["ticket_id"] is None
    assert payload["missing_arguments"] == ["diagnostic_id"]
    assert payload["blocking_reasons"] == ["missing required recovery ticket argument: diagnostic_id"]


def _install_scheduler_handler_guards(monkeypatch) -> None:
    def fail(*_args: Any, **_kwargs: Any) -> object:
        raise AssertionError("attempt-run-recovery-ticket-intent must not run scheduler handlers")
...
def _intervention_run(**overrides) -> Phase5SchedulerAttemptInterventionRunArtifact:
    payload = {
        "intervention_run_id": "intervention-run-cg-cli",
        "cycle_id": "cycle-cg-cli",
        "runner_id": "runner-cg-cli",
        "issued_at": "2026-05-21T10:00:00Z",
        "execution_status": "applied",
        "applied_output": "diagnostic",
        "action": "open_recovery_ticket",
        "diagnostic_id": "diagnostic-cg-cli",
