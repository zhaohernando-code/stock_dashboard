...
def test_attempt_run_readout_output_reads_artifacts_without_running_scheduler(
    tmp_path: Path,
    monkeypatch,
    capsys,
) -> None:
    ...
    write_phase5_scheduler_attempt_run_artifact(
        _attempt_run(run_id="run-old", issued_at="2026-05-21T09:00:00Z", apply_status="blocked"),
        root=artifact_root,
    )
    write_phase5_scheduler_attempt_run_artifact(
        _attempt_run(run_id="run-new", issued_at="2026-05-21T10:00:00Z"),
        root=artifact_root,
    )
    write_phase5_scheduler_attempt_run_artifact(
        _attempt_run(run_id="run-other", cycle_id="cycle-other", issued_at="2026-05-21T11:00:00Z"),
        root=artifact_root,
    )
    ...
    assert payload["cycle_id"] == "cycle-readout"
    assert payload["runner_id"] == "runner-bw1"
    assert payload["total_runs"] == 2
    assert payload["latest_run_id"] == "run-new"
    assert payload["latest_blocked_run_id"] == "run-old"
    assert payload["staleness_status"] == "current"


def test_attempt_run_readout_output_handles_empty_store(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert payload["staleness_status"] == "degraded"
    assert payload["run_refs"] == []


def _install_scheduler_handler_guards(monkeypatch) -> None:
    def fail(*_args: Any, **_kwargs: Any) -> object:
        raise AssertionError("attempt-run-readout must not run scheduler handlers")
...
def _attempt_run(**overrides) -> Phase5SchedulerAttemptRunArtifact:
    payload = {
        "run_id": "run-bw1",
        "attempt_id": "attempt-bw1",
        "cycle_id": "cycle-readout",
        "runner_id": "runner-bw1",
        ...
        "missing_arguments": [],
        "diagnostic_id": None,
        "execution_id": "execution-bw1",
        "idempotency_key": "cycle:cycle-readout:retry_failed_step",
