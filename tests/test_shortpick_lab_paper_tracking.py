...
class ShortpickLabPaperTrackingTests(ShortpickLabTestCase):
    ...
    def test_market_factor_paper_controls_get_same_exit_tracks(self) -> None:
        ...
        archived_roles = {item["role"] for item in contract["archived_controls"]}
        self.assertIn(SHORTPICK_MARKET_FACTOR_OFFENSIVE_TOP1_CONTROL_ROLE, archived_roles)
        self.assertIn(SHORTPICK_MARKET_FACTOR_COOLDOWN_TOP1_CONTROL_ROLE, archived_roles)
        self.assertIn(SHORTPICK_MARKET_FACTOR_TOP3_EQUAL_WEIGHT_CONTROL_ROLE, archived_roles)
        self.assertIn(SHORTPICK_MARKET_FACTOR_NO_LIMIT_CHASE_LOW_TURNOVER_CONTROL_ROLE, archived_roles)
        for role in (
            SHORTPICK_MARKET_FACTOR_RANDOM_POOL_CONTROL_ROLE,
            SHORTPICK_MARKET_FACTOR_OPEN_ENTRY_LOW_TURNOVER_CONTROL_ROLE,
            SHORTPICK_MARKET_FACTOR_SAME_SYMBOL_COOLDOWN_LOW_TURNOVER_CONTROL_ROLE,
