...
def test_action_route_apply_output_calls_core_apply_only(
    ...
    def fake_apply(plan: _FakePlanResult, route: _FakeResult, **kwargs: Any) -> _FakeResult:
        calls.append("apply")
    ...
    def fail_dry_run(_plan: _FakePlanResult) -> object:
        raise AssertionError("action-route-apply output should not call dry-run executor")

    def fail_diagnostic(_plan: _FakePlanResult, **_kwargs: Any) -> object:
        raise AssertionError("action-route-apply output should not call diagnostic recorder")

    def fail_execution(_plan: _FakePlanResult, **_kwargs: Any) -> object:
        raise AssertionError("action-route-apply output should not call execution ledger recorder")

    def fail_preflight(*_args: Any, **_kwargs: Any) -> object:
        raise AssertionError("action-route-apply output should not call route preflight directly")

    def fail_service(**_kwargs: Any) -> _FakeServiceResult:
        raise AssertionError("action-route-apply output should not call full service")
    ...
    monkeypatch.setattr(cli_autonomous_flow, "apply_phase5_scheduler_action_route", fake_apply)
    ...
    monkeypatch.setattr(cli_autonomous_flow, "preflight_phase5_scheduler_action_route", fail_preflight)
    ...
    result = cli_autonomous_flow.handle_phase5_local_cycle_step_command(
        _args(
            output="action-route-apply",
            cycle_id="cycle-route-apply",
            gate_id="gate-1",
    ...
    assert calls == ["tick", "plan", "action", "route", "apply"]
    _assert_rich_tick_args(tick_calls, cycle_id="cycle-route-apply", root=artifact_root)
    assert apply_inputs == [
        {
            "plan": _plan_result(cycle_id="cycle-route-apply", action="retry_failed_step").payload,
            "route": _result(cycle_id="cycle-route-apply", route_type="execution_output").payload,
            "diagnostic_id": "diagnostic-1",
            "observed_at": "2026-05-20T10:01:00Z",
            "execution_id": "execution-1",
            "idempotency_key": "idempotency:execution-1",
...
def test_action_route_apply_output_blocks_missing_args_without_writes(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    assert payload["missing_arguments"] == ["diagnostic_id", "observed_at"]
    for generated_name in ("observed_at", "created_at"):
