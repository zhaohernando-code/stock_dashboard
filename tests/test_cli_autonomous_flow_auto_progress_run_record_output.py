...
def test_auto_progress_apply_output_records_run_when_enabled(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    exit_code = cli_module.main(
        [
            ...
            "--record-auto-progress-run",
            "--auto-progress-run-id",
            "auto-progress-run-cm-cli",
        ]
    )

    payload = json.loads(capsys.readouterr().out)
    stored = read_phase5_scheduler_auto_progress_run_artifact("auto-progress-run-cm-cli", root=artifact_root)
    assert exit_code == 0
    assert payload["auto_progress_run_record_status"] == "recorded"
    assert stored.phase == "recovery_followup_apply"
    assert stored.apply_status == "applied"
    assert stored.result_refs == payload["auto_progress_run_artifact"]["result_refs"]


def test_auto_progress_apply_output_record_blocks_missing_context(tmp_path: Path, monkeypatch, capsys) -> None:
    artifact_root = tmp_path / "artifacts"
    _guard_init_database(monkeypatch)
    _install_scheduler_handler_guards(monkeypatch)
    _record_ticket(artifact_root)
    ...
    payload = json.loads(capsys.readouterr().out)
    assert exit_code == 0
    assert payload["auto_progress_run_record_status"] == "blocked"
    assert payload["auto_progress_run_artifact"] is None
    assert payload["auto_progress_run_record_missing_arguments"] == ["issued_at"]
...
def _record_ticket(root: Path) -> None:
    ...
    record_phase5_recovery_ticket(
        cycle_id="cycle-cm-cli",
        ticket_id="ticket-cm-cli",
        failed_step="replay_schedule",
        failure_class="contract_violation",
        failure_observed_at="2026-05-21T10:00:00Z",
        evidence_refs=["phase5_scheduler_diagnostic:diagnostic-cm-cli"],
