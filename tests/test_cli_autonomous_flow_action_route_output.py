...
def test_phase5_local_cycle_step_action_route_output_calls_router_only(
    ...
    def fake_route(action_result: _FakeActionResult) -> _FakeRouteResult:
        calls.append("route")
        return _route_result(cycle_id=action_result.payload["cycle_id"])

    def fail_dry_run(_plan: _FakePlanResult) -> object:
        raise AssertionError("action-route output should not call dry-run executor")

    def fail_diagnostic(_plan: _FakePlanResult, **_kwargs: Any) -> object:
        raise AssertionError("action-route output should not call diagnostic recorder")

    def fail_execution(_plan: _FakePlanResult, **_kwargs: Any) -> object:
        raise AssertionError("action-route output should not call execution ledger recorder")

    def fail_service(**_kwargs: Any) -> _FakeServiceResult:
        raise AssertionError("action-route output should not call full service")
    ...
    assert exit_code == 0
    assert calls == ["tick", "plan", "action", "route"]
    _assert_rich_tick_args(tick_calls, cycle_id="cycle-action-route", root=artifact_root)
    assert json.loads(capsys.readouterr().out) == _route_result(cycle_id="cycle-action-route").payload


def test_phase5_local_cycle_step_action_route_missing_cycle_routes_to_diagnostic_without_writes(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    assert payload == {
        ...
        "reason": "scheduler action preflight blocked by missing inputs",
    }
    for generated_name in ("diagnostic_id", "observed_at", "execution_id", "idempotency_key", "created_at"):
...
def _route_result(*, cycle_id: str) -> _FakeRouteResult:
