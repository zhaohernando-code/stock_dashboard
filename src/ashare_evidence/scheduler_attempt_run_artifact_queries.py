...
def list_phase5_scheduler_attempt_run_artifacts(
    *,
...
) -> list[Phase5SchedulerAttemptRunArtifact]:
    ...
    artifacts: list[Phase5SchedulerAttemptRunArtifact] = []
    for path in directory.glob("*.json"):
        if not path.is_file():
            continue
        artifact = Phase5SchedulerAttemptRunArtifact.model_validate(json.loads(path.read_text(encoding="utf-8")))
        if all(expected is None or getattr(artifact, field) == expected for field, expected in filters):
            artifacts.append(artifact)
    return sorted(artifacts, key=lambda artifact: (artifact.issued_at, artifact.run_id), reverse=True)


def find_latest_phase5_scheduler_attempt_run_artifact(
    *,
    cycle_id: str | None = None,
    runner_id: str | None = None,
    attempt_status: str | None = None,
    ...
    artifacts = list_phase5_scheduler_attempt_run_artifacts(
        cycle_id=cycle_id,
        runner_id=runner_id,
        attempt_status=attempt_status,
        apply_status=apply_status,
