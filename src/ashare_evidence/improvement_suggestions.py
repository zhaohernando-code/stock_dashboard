...
def _build_control_plane_task_description(item: dict[str, Any], *, selected_model: str) -> str:
    ...
    return "\n".join(
        [
            "这是股票看板改进建议审计台转入计划池的任务。请先进入 Plan 模式生成可审视计划。",
            "计划只面向业务结果、风险边界、验收方式和是否需要用户拍板的真实事项；文件、字段、接口、测试命令等实现细节由 AI 自行判断，不要作为用户问题暴露。",
            "",
            "关键约束：",
            ...
            "",
            "本次改进目标：",
            f"- 标的范围：{item.get('symbol') or '全局'}",
            f"- 用户可见问题：{item.get('claim') or ''}",
            f"- 期望结果：{item.get('proposed_change') or ''}",
            f"- 证据状态：{item.get('evidence_status')}",
            f"- 执行模型：{selected_model}",
            "",
            "计划生成要求：",
            "- 明确用户最后要验收什么页面或 API 行为。",
            "- 明确哪些交易/研究策略边界不能被本任务改变。",
            "- 明确需要发布和真实页面验收。",
            "- 不要求用户选择代码路径、数据字段、测试文件或实现方案。",
            "",
            "AI 内部参考，不要原样转换成用户待回答问题：",
