...
from ashare_evidence.event_confirmed_position_exit import (
    run_event_confirmed_position_exit_ablation,
    write_event_confirmed_position_exit_result,
)


def main() -> int:
    parser = argparse.ArgumentParser(description="Run the preregistered event-confirmed V3 position-exit ablation.")
    for name in (
        "execution-snapshot",
    ...
    payload = run_event_confirmed_position_exit_ablation(
    ...
    write_event_confirmed_position_exit_result(args.output, payload)
    print(
        json.dumps(
            {
