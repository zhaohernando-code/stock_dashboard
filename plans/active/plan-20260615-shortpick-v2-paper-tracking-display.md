...
| W-002 | pending | 2 | W-001 | Rework the `试验田v2` paper tab to use the v1-style display structure and Chinese-readable labels, with no raw field-shaped visible content. | `frontend/src/components/ShortpickLabV2View.tsx`, types/helpers/styles/static tests | test_pass | cmd:python3 -m pytest -q tests/test_frontend_shortpick_static.py && cd frontend && npm run build | |
