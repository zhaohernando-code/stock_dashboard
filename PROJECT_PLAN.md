...
- 短投试验田新增独立计划 `SHORTPICK_LAB_MULTI_BENCHMARK_FEEDBACK_PLAN.md`：收益反馈要从单一沪深300口径升级为 `沪深300 / 中证1000 / 同板块等权` 三维 benchmark，并在表格收益列的表头进行切换。该计划只覆盖试验田后验验证与模型反馈展示，不回写主 recommendation、不调整模拟盘或生产权重；缺中证行情、缺板块映射或同行样本不足时必须显示待补原因，不能用绝对收益或沪深300代替。
- 当前活动状态以 `PROJECT_STATUS.json`、`PROCESS.md` 和 `docs/contracts/` / `docs/archive/` 下的最新材料为准；旧阶段的完成记录只保留在执行日志中，不再作为默认开发路线展示。
- 截至 `2026-08-08`，外部信息 full-window 数据固化与多轮账户回放研究已完成阶段性收口：CNINFO 官方公告、全球行情、申万一级行业、宏观和行业资金流均已形成 PIT-safe 离线研究快照；静态权重、规则门控、持有期、Rank1/Rank2 预算迁移、Top20 残差重排与 Top5 近似同分破平均未找到同时满足八项非劣化指标和突出优势门槛的方案。V3 保持永久 `lambda=0`，外部层不进入纸面或生产；下一次晋升研究只在合格专业新闻/正文 surprise、供应商 revision/vintage 或新独立时间留出段到位后启动。

Current step: `Phase 5 - Real Validation Rebuild and Model Portfolio Policy Research` remains in progress; the P-1/P0 professionalization tranche is implemented, published to the local runtime, and fully closed through tests, deploy verifier, and served-browser verification. The latest closeout has `pytest -q` at `212 passed`, deploy verification at `19 passed / 0 failed`, and a Chrome served-page check with zero console errors. Summary-first operations loading, lazy detail loading, data-quality, benchmark, factor-IC, weight-sweep, and product-facing validation fields are now the active contract. Replay time-window accumulation, holding-policy breadth, horizon approval, production weight calibration, and shortpick multi-benchmark feedback remain open.
